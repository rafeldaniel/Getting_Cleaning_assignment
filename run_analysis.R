...
# 1 Merges the training and the test sets to create one data set called x.
# 2 Extracts only the measurements on the mean and standard deviation for each measurement.
# 3Uses descriptive activity names to name the activities in the data set
# 4 Appropriately labels the data set with descriptive variable names.
# 5 From the data set in step 4, creates a second, independent tidy data set called sm with the average of each variable for each 
...
setwd("~/Rafa/RWorkingDirecory/data/project/UCI HAR Dataset")
...
write.table(x,"X_only_completeset")

x<-mergeSignals(x)
write.table(x,"completeset")
